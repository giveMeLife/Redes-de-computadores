...
def graphicFourier(data,rate,dataAM,rateAM,dataFM,rateFM,dataDemodulada,percentage):
	...
	fc = 3*rate
	time_interpol = np.linspace(0,len(data)/rate,num=len(data))
	coseno = np.cos(2*np.pi*fc*time_interpol)
	trasnP,freqP = Fourier(coseno,fc)
	fig = plt.figure()
	fig.subplots_adjust(hspace=0.7, wspace=0.7)
	fig.add_subplot(411)
	plt.title("Señal Original")
...
#graphicFourier(data_interpol,rate,dataAM_1,rateAM_1,dataFM_1,rateFM_1,filtered_1,"100%")
...
wavfile.write("filtradoBajo2.wav",rate,filtered_15)
...
wavfile.write("filtradoBajo3.wav",rate,filtered_125)
